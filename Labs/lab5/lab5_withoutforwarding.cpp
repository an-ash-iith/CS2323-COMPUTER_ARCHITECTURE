...
//checking for s1
  ...
  if (s1[0] != 's' || s1[1] != 'd')
  {
    for (int i = 0; i < s1.size(); i++)
    {
      ...
      if (s1[i] == 'x') // lower case x as naming of regisgter
      {
         if(i<s1.size()-2 && isdig(s1[i+2]) )
        {
          destination= (s1[i+1]-'0')*10+(s1[i+2]-'0');
        }
        else
        destination = s1[i + 1]-'0';
        break;
      }
    }
  }
...
cout<<"Enter the number of instruction"<<endl;
  ...
  for (int i = 1; i < n; i++)
  {
    ...
    else if (checkhazard(s1, s2, s3) == 1)
    {
